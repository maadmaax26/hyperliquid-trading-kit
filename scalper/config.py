...
BTC_CONFIG = AssetConfig(
    coin="BTC",
    take_profit_pct=0.0021,         # V5 OPT: 0.42%→0.21% — tight TP, trailing does the work (81% WR, PF 3.44)
    stop_loss_pct=0.0025,           # Keep 0.25% SL
    trailing_activate_pct=0.0015,   # V5: 0.30%→0.15% — activate trailing at first sign of profit
    trailing_stop_pct=0.0004,       # V5: 0.15%→0.04% — very tight trail, lock in micro-profits
...
ETH_CONFIG = AssetConfig(
    coin="ETH",
    take_profit_pct=0.0025,         # V5 OPT: 0.50%→0.25% — tight TP (76% WR, PF 2.85)
    stop_loss_pct=0.0030,           # Keep 0.30% SL
    trailing_activate_pct=0.0017,   # V5: 0.35%→0.17% — early trailing activation
    trailing_stop_pct=0.0004,       # V5: 0.18%→0.04% — tight trail
...
SOL_CONFIG = AssetConfig(
    coin="SOL",
    take_profit_pct=0.0030,         # V5 OPT: 0.60%→0.30% — tight TP (74% WR, PF 2.44)
    stop_loss_pct=0.0040,           # Keep 0.40% SL
    trailing_activate_pct=0.0023,   # V5: 0.40%→0.23% — early trailing
    trailing_stop_pct=0.0006,       # V5: 0.20%→0.06% — tight trail
...
XRP_CONFIG = AssetConfig(
    coin="XRP",
    take_profit_pct=0.0023,         # V5 OPT: 0.45%→0.23% — tight TP (79% WR, PF 4.73)
    stop_loss_pct=0.0025,           # Keep 0.25% SL
    trailing_activate_pct=0.0017,   # V5: 0.30%→0.17% — early trailing
    trailing_stop_pct=0.0004,       # V5: 0.15%→0.04% — tight trail
    position_size_pct=0.20,
    min_signal_score=10,
    cooldown_seconds=120,
    max_spread_pct=0.001,
...
ZEC_CONFIG = AssetConfig(
    coin="ZEC",
    take_profit_pct=0.0028,         # V5 OPT: 0.55%→0.28% — tight TP (80% WR, PF 5.60)
    stop_loss_pct=0.0030,           # Keep 0.30% SL
    trailing_activate_pct=0.0020,   # V5: 0.40%→0.20% — early trailing (729 trail exits, 100% WR)
    trailing_stop_pct=0.0006,       # V5: 0.20%→0.06% — tight trail, lock in profits fast
    position_size_pct=0.20,         # Moderate size — ZEC is high-volatility
...
class BotConfig:
    ...
    max_concurrent_positions: int = 3     # V5: Allow 3 (higher-quality filtered trades, more diversification)
